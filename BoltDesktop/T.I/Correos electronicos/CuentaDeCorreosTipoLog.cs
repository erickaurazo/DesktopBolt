...
{
    public partial class CuentaDeCorreosTipoLog : Form
    {
        ...
        private SAS_CuentasCorreoTipoLog selectedItem;
        private List<SAS_CuentasCorreoTipoLog> resultList;
        SAS_CuentasCorreoController model;
        SAS_CuentasCorreoTipoLog itemRegistar;
        SAS_CuentasCorreoTipoLog itemDelete;
        ...
        public CuentaDeCorreosTipoLog(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
            connection = "SAS";
            userLogin = new SAS_USUARIOS();
        ...
        private void CuentaDeCorreosTipoLog_Load(object sender, EventArgs e)
        {
        ...
        private void CuentaDeCorreosTipoLog_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new  SAS_CuentasCorreoTipoLog();
                    itemRegistar.CuentaCorreoTipoLogID = Convert.ToInt32(this.txtCodigo.Text);
                    itemRegistar.Descripcion = this.txtDescripcion.Text.Trim();
                    itemRegistar.Estado = this.txtIdEstado.Text.Trim() == "1" ? Convert.ToDecimal("1") : Convert.ToDecimal("0");

                    model = new SAS_CuentasCorreoController();
                    if (model.RegistrarTipoCuentaCorreoLog(connection, itemRegistar) >= 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.CuentaCorreoTipoLogID = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_CuentasCorreoController();
                    if (model.EliminarTipoCuentaCorreoLog(connection, itemRegistar) >= 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.CuentaCorreoTipoLogID = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_CuentasCorreoController();
                    if (model.CambiarEstadoTipoCuentaCorreoLog(connection, itemRegistar) >= 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CuentasCorreoTipoLog>();
                model = new SAS_CuentasCorreoController();
                resultList = model.ListadoTipoCuentaCorreoLog(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CuentasCorreoTipoLog();
            selectedItem.CuentaCorreoTipoLogID = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.CuentaCorreoTipoLogID.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CuentasCorreoTipoLog();
                        selectedItem = resultByFilterId.ElementAt(0);

                        txtCodigo.Text = selectedItem.CuentaCorreoTipoLogID.ToString();
