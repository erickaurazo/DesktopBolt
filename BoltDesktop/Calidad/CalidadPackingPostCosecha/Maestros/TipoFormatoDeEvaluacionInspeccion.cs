...
{
    public partial class TipoFormatoDeEvaluacionInspeccion : Form
    {
        #region Variables() 
        private int periodo;
        ...
        private SAS_CALIDAD_TIPO_REGISTRO selectedItem;
        private List<SAS_CALIDAD_TIPO_REGISTRO> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_TIPO_REGISTRO itemRegistar;
        SAS_CALIDAD_TIPO_REGISTRO itemDelete;
        ...
        public TipoFormatoDeEvaluacionInspeccion()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public TipoFormatoDeEvaluacionInspeccion(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void TipoFormatoDeEvaluacionInspeccion_Load(object sender, EventArgs e)
        {
        ...
        private void TipoFormatoDeEvaluacionInspeccion_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_TIPO_REGISTRO();
            selectedItem.idTipoRegistro = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idTipoRegistro.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_TIPO_REGISTRO();
                        selectedItem = resultByFilterId.ElementAt(0);
                        txtCodigo.Text = selectedItem.idTipoRegistro.ToString();
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0" )
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_TIPO_REGISTRO();
                    itemRegistar.idTipoRegistro = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    if (model.ToRegisterGrupo(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idTipoRegistro = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteGrupo(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idTipoRegistro = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusGrupo(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_TIPO_REGISTRO>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllGrupo(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
