...
{
    public partial class ActividadLavadoDeManos : Form
    {
        ...
        private SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS selectedItem;
        private List<SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS itemRegistar;
        SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS itemDelete;
        ...
        public ActividadLavadoDeManos(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void ActividadLavadoDeManos_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS();
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        ...
                        txtCodigo.Text = selectedItem.idActividad.ToString();
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS();
                    ...
                    if (model.ToRegisterActividadLavadoDeManos(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idActividad = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteActividadLavadoDeManos(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idActividad = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusActividadLavadoDeManos(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_ACTIVIDAD_LAVADO_MANOS>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllActividadLavadoDeManos(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
        ...
        private void ActividadLavadoDeManos_Load(object sender, EventArgs e)
        {
