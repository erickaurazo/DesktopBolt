...
{
    public partial class ActividadesAplicacionSO2 : Form
    {
        ...
        private SAS_AplicacionSO2ActividadesAll selectedItem;
        private List<SAS_AplicacionSO2ActividadesAll> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_FORM_APLICACION_SO2_ACTIVIDADES itemRegistar;
        SAS_CALIDAD_FORM_APLICACION_SO2_ACTIVIDADES itemDelete;
        ...
        public ActividadesAplicacionSO2()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public ActividadesAplicacionSO2(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void ActividadesAplicacionSO2_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_AplicacionSO2ActividadesAll();
            selectedItem.idActividad = 0;
            selectedItem.idSubProceso = 0;
            selectedItem.descripcion = string.Empty;
            selectedItem.SubProceso = string.Empty;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idActividad.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_AplicacionSO2ActividadesAll();
                        selectedItem = resultByFilterId.ElementAt(0);
                        this.txtCodigo.Text = selectedItem.idSubProceso.ToString();
                        ...
                        this.txtSubProcesoId.Text = selectedItem.idSubProceso != null ? selectedItem.idSubProceso.ToString() : string.Empty;
                        this.txtSubProceso.Text = selectedItem.SubProceso != null ? selectedItem.SubProceso.ToString() : string.Empty;
                    }
                }
            }
            #endregion 
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_FORM_APLICACION_SO2_ACTIVIDADES();
                    itemRegistar.idActividad = Convert.ToInt32(this.txtCodigo.Text);
                    itemRegistar.idSubProceso = Convert.ToInt32(this.txtSubProcesoId.Text);
                    ...
                    if (model.ToRegisteActividadesSO2(connection, itemRegistar) > 0)
                    {
        ...
        private bool ValidateForm()
        {
            ...
            if (this.txtSubProcesoId.Text.Trim() == string.Empty)
            {
                status = false;
            }
            ...
            if (this.txtSubProceso.Text.Trim() == string.Empty)
            {
                status = false;
            }
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idActividad = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteActividadesSO2(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idActividad = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusActividadesSO2(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_AplicacionSO2ActividadesAll>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllActividadesSO2(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
