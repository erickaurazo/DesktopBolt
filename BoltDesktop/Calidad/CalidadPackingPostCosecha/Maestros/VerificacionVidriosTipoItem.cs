...
{
    public partial class VerificacionVidriosTipoItem : Form
    {
        ...
        private SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem selectedItem;
        private List<SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem itemRegistar;
        SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem itemDelete;
        ...
        public VerificacionVidriosTipoItem()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public VerificacionVidriosTipoItem(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void VerificacionVidriosTipoItem_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem();
            selectedItem.TipoItem = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.TipoItem.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem();
                        selectedItem = resultByFilterId.ElementAt(0);
                        txtCodigo.Text = selectedItem.TipoItem.ToString();
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem();
                    itemRegistar.TipoItem = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    if (model.ToRegisterVerificacionVidriosItem(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.TipoItem = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteVerificacionVidriosItem(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.TipoItem = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusVerificacionVidriosItem(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_FORM_VERIFICACION_VIDRIOS_TipoItem>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllVerificacionVidriosItem(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
