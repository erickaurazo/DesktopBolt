...
{
    public partial class InocuidadCriterios : Form
    {
        ...
        private SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS selectedItem;
        private List<SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS itemRegistar;
        SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS itemDelete;
        ...
        public InocuidadCriterios(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS();
        ...
        private void InocuidadCriterios_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS();
                    ...
                    if (model.ToRegisterInocuidadCriterios(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idCriterio = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteInocuidadCriterios(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idCriterio = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusInocuidadCriterios(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            {
                #region Consultar()
                resultList = new List<SAS_CALIDAD_FORM_INOCUIDAD_CRITERIOS>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllInocuidadCriterios(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
