...
{
    public partial class AmbientesLuminariasVidriosPlasticos : Form
    {
        ...
        private SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS selectedItem;
        private List<SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS itemRegistar;
        SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS itemDelete;
        ...
        public AmbientesLuminariasVidriosPlasticos()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public AmbientesLuminariasVidriosPlasticos(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS();
            selectedItem.idAmbiente = 0;
            selectedItem.descripcion = string.Empty;
            selectedItem.cantLuminarias = 0;
            selectedItem.cantPlasticos = 0;
            selectedItem.cantVidrios = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idAmbiente.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS();
                        selectedItem = resultByFilterId.ElementAt(0);

                        txtCodigo.Text = selectedItem.idAmbiente.ToString();
                        txtDescripcion.Text = selectedItem.descripcion.ToString();
                        txtIdEstado.Text = selectedItem.estado.ToString();
                        txtEstado.Text = selectedItem.estado.ToString() == "1" ? "ACTIVO" : "ANULADO";
                        this.txtLuminarias.Text = selectedItem.cantLuminarias != null ? selectedItem.cantLuminarias.ToString() : string.Empty;
                        this.txtVidrios.Text = selectedItem.cantVidrios != null ? selectedItem.cantVidrios.ToString() : string.Empty;
                        this.txtPlasticos.Text = selectedItem.cantPlasticos != null ? selectedItem.cantPlasticos.ToString() : string.Empty;
                    }
                }
            }
            #endregion 
        ...
        private void AmbientesLuminariasVidriosPlasticos_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void AmbientesLuminariasVidriosPlasticos_Load(object sender, EventArgs e)
        {
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS();
                    itemRegistar.idAmbiente = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    itemRegistar.cantLuminarias = Convert.ToInt32(this.txtLuminarias.Text);
                    itemRegistar.cantVidrios = Convert.ToInt32(this.txtVidrios.Text);
                    itemRegistar.cantPlasticos = Convert.ToInt32(this.txtPlasticos.Text);


                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToRegisterAmbientesLuminariaVidriosPlasticos(connection, itemRegistar) > 0)
                    {
        ...
        private bool ValidateForm()
        {
            ...
            if (this.txtLuminarias.Text.Trim() == string.Empty)
            {
                status = false;
            }


            if (this.txtVidrios.Text.Trim() == string.Empty)
            {
                status = false;
            }
            ...
            if (this.txtPlasticos.Text.Trim() == string.Empty)
            {
                status = false;
            }
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idAmbiente = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteAmbientesLuminariaVidriosPlasticos(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idAmbiente = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusAmbientesLuminariaVidriosPlasticos(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_AMBIENTES_LUMINARIAS_VIDRIOS_PLASTICOS>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllAmbientesLuminariaVidriosPlasticos(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
