...
{
    public partial class SubProcesoSo2 : Form
    {
        #region Variables() 
        private int periodo;
        ...
        private SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO selectedItem;
        private List<SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO itemRegistar;
        SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO itemDelete;
        ...
        public SubProcesoSo2(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void SubProcesoSo2_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO();
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idSubProceso.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO();
                        selectedItem = resultByFilterId.ElementAt(0);

                        txtCodigo.Text = selectedItem.idSubProceso.ToString();
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO();
                    itemRegistar.idSubProceso = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    if (model.ToRegisterSubProcesoSO2(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idSubProceso = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteSubProcesoSO2(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idSubProceso = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusSubProcesoSO2(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_FORM_APLICACION_SO2_SUBPROCESO>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllSubProcesoSO2(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
        ...
        private void SubProcesoSo2_Load(object sender, EventArgs e)
        {
