...
{
    public partial class LiberacionDeLineas : Form
    {
        #region Variables() 
        private int periodo;
        ...
        private SAS_ListadoItemsParaLiberacionDeLineasAll selectedItem;
        private List<SAS_ListadoItemsParaLiberacionDeLineasAll> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_ITEMS_LIBERACION_LINEAS itemRegistar;
        SAS_CALIDAD_ITEMS_LIBERACION_LINEAS itemDelete;
        ...
        public LiberacionDeLineas(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void LiberacionDeLineas_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_ListadoItemsParaLiberacionDeLineasAll();
            selectedItem.ItemLiberacionId = 0;
            selectedItem.ItemLiberacion = string.Empty;
            selectedItem.EstadoId = Convert.ToChar("0");
            selectedItem.AreaDeInspeccionId = 0;
            selectedItem.AreaDeInspeccion = string.Empty;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.ItemLiberacionId.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_ListadoItemsParaLiberacionDeLineasAll();
                        selectedItem = resultByFilterId.ElementAt(0);

                        txtCodigo.Text = selectedItem.ItemLiberacionId.ToString();
                        txtDescripcion.Text = selectedItem.ItemLiberacion.ToString();
                        txtIdEstado.Text = selectedItem.EstadoId.ToString();
                        txtEstado.Text = selectedItem.Estado;
                        this.txtArea.Text = selectedItem.AreaDeInspeccion;
                        this.txtAreaCodigo.Text = selectedItem.AreaDeInspeccionId.Value.ToString();
        ...
        private void LiberacionDeLineas_Load(object sender, EventArgs e)
        {
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_ITEMS_LIBERACION_LINEAS();
                    itemRegistar.idItem = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    itemRegistar.idArea = Convert.ToInt32(this.txtAreaCodigo.Text.ToString().Trim());
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToRegisterLiberacionLineas(connection, itemRegistar) > 0)
                    {
        ...
        private bool ValidateForm()
        {
            ...
            if (this.txtAreaCodigo.Text.Trim() == string.Empty)
            {
                status = false;
            }


            if (this.txtArea.Text.Trim() == string.Empty)
            {
                status = false;
            }
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idItem = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteLiberacionLineas(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idItem = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusLiberacionLineas(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_ListadoItemsParaLiberacionDeLineasAll>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllLiberacionLineas(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
        ...
        private void PintarResultadosEnGrilla()
        {
            ...
            if ((ClickResaltarResultados % 2) == 0)
            {
                ...
                dgvRegistro.Columns["chEstadoId"].ConditionalFormattingObjectList.Add(c1);
                ...
                dgvRegistro.Columns["chEstadoId"].ConditionalFormattingObjectList.Add(c4);
                #endregion
            }
            else
            {
