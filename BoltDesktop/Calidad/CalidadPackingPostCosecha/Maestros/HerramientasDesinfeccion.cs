...
{
    public partial class HerramientasDesinfeccion : Form
    {
        #region Variables() 
        private int periodo;
        ...
        private SAS_CALIDAD_HERRAMIENTAS_DESINFECCION selectedItem;
        private List<SAS_CALIDAD_HERRAMIENTAS_DESINFECCION> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_HERRAMIENTAS_DESINFECCION itemRegistar;
        SAS_CALIDAD_HERRAMIENTAS_DESINFECCION itemDelete;
        ...
        public HerramientasDesinfeccion()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public HerramientasDesinfeccion(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void HerramientasDesinfeccion_Load(object sender, EventArgs e)
        {
        ...
        private void HerramientasDesinfeccion_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_HERRAMIENTAS_DESINFECCION();
            selectedItem.idHerramienta = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idHerramienta.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_HERRAMIENTAS_DESINFECCION();
                        selectedItem = resultByFilterId.ElementAt(0);

                        txtCodigo.Text = selectedItem.idHerramienta.ToString();
                        ...
                        txtEstado.Text = selectedItem.estado != (char?)null ? selectedItem.estado.ToString() : "0";
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_HERRAMIENTAS_DESINFECCION();
                    itemRegistar.idHerramienta = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    if (model.ToRegisterHerramientasDesinfeccion(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idHerramienta = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteHerramientasDesinfeccion(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idHerramienta = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusHerramientasDesinfeccion( connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_HERRAMIENTAS_DESINFECCION>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllHerramientasDesinfeccion(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
