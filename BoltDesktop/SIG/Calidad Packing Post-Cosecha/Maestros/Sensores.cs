...
namespace ComparativoHorasVisualSATNISIRA.Calidad.CalidadPackingPostCosecha.Maestros
{
    public partial class Sensores : Form
    {
        ...
        private SAS_CALIDAD_SENSORES selectedItem;
        private List<SAS_CALIDAD_SENSORES> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_SENSORES itemRegistar;
        SAS_CALIDAD_SENSORES itemDelete;
        ...
        public Sensores(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void Sensores_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_SENSORES();
            selectedItem.idSensor = 0;
            selectedItem.descripcion = string.Empty;
            selectedItem.tipo = string.Empty;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idSensor.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_SENSORES();
                        selectedItem = resultByFilterId.ElementAt(0);

                        txtCodigo.Text = selectedItem.idSensor.ToString();
                        ...
                        this.txtTipo.Text = selectedItem.tipo.ToString();
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_SENSORES();
                    itemRegistar.idSensor = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    itemRegistar.tipo = (this.txtTipo.Text.ToString().Trim());
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToRegisterSensores(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idSensor = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteSensores(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idSensor = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusSensores(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_SENSORES>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllSensores(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
