...
{
    public partial class FrecuenciaFormatoDeEvaluacionInspeccion : Form
    {
        #region Variables() 
        private int periodo;
        ...
        private SAS_CALIDAD_FRECUENCIA_REGISTRO selectedItem;
        private List<SAS_CALIDAD_FRECUENCIA_REGISTRO> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_FRECUENCIA_REGISTRO itemRegistar;
        SAS_CALIDAD_FRECUENCIA_REGISTRO itemDelete;
        ...
        #endregion
        public FrecuenciaFormatoDeEvaluacionInspeccion()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public FrecuenciaFormatoDeEvaluacionInspeccion(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void FrecuenciaFormatoDeEvaluacionInspeccion_Load(object sender, EventArgs e)
        {
        ...
        private void FrecuenciaFormatoDeEvaluacionInspeccion_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_FRECUENCIA_REGISTRO();
            selectedItem.idFrecuencia = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idFrecuencia.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_FRECUENCIA_REGISTRO();
                        selectedItem = resultByFilterId.ElementAt(0);
                        txtCodigo.Text = selectedItem.idFrecuencia.ToString();
                        txtDescripcion.Text = selectedItem.descripcion.ToString();
                        txtIdEstado.Text = selectedItem.estado.Value.ToString();
                        txtEstado.Text = ((selectedItem.estado.Value != null ? selectedItem.estado.Value.ToString() : "0") == "1" ? "ACTIVO" : "ANULADO");
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_FRECUENCIA_REGISTRO();
                    itemRegistar.idFrecuencia = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    if (model.ToRegisterFrecuenciaInspeccion(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idFrecuencia = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteFrecuenciaInspeccion(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idFrecuencia = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusFrecuenciaInspeccion(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_FRECUENCIA_REGISTRO>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllFrecuenciaInspeccion(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
