...
{
    public partial class CriteriosManufactura : Form
    {
        ...
        private SAS_CALIDAD_CRITERIOS_MANUFACTURA selectedItem;
        private List<SAS_CALIDAD_CRITERIOS_MANUFACTURA> resultList;
        SAS_FormatosDeInspeccionCalidadPacking model;
        SAS_CALIDAD_CRITERIOS_MANUFACTURA itemRegistar;
        SAS_CALIDAD_CRITERIOS_MANUFACTURA itemDelete;
        ...
        public CriteriosManufactura(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void dgvRegistro_SelectionChanged(object sender, EventArgs e)
        {
            ...
            selectedItem = new SAS_CALIDAD_CRITERIOS_MANUFACTURA();
            selectedItem.idCriterio = 0;
            ...
            if (dgvRegistro.Rows.Count > 0)
            {
                if (dgvRegistro.CurrentRow != null && dgvRegistro.CurrentRow.Cells["chId"].Value != null)
                {
                    ...
                    var resultByFilterId = resultList.Where(x => x.idCriterio.ToString().Trim() == idSelect).ToList();
                    if (resultByFilterId.ToList().Count == 1)
                    {
                        selectedItem = new SAS_CALIDAD_CRITERIOS_MANUFACTURA();
                        selectedItem = resultByFilterId.ElementAt(0);
                        txtCodigo.Text = selectedItem.idCriterio.ToString();
        ...
        private void CriteriosManufactura_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.bgwHilo.IsBusy == true)
            {
                e.Cancel = true;
        ...
        private void CriteriosManufactura_Load(object sender, EventArgs e)
        {
        ...
        private void Registrar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                ...
                {
                    itemRegistar = new SAS_CALIDAD_CRITERIOS_MANUFACTURA();
                    itemRegistar.idCriterio = Convert.ToInt32(this.txtCodigo.Text);
                    ...
                    if (model.ToRegisterCriteriosManufactura(connection, itemRegistar) > 0)
                    {
        ...
        private void Eliminar()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty && this.txtIdEstado.Text.Trim() != "0")
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idCriterio = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToDeleteCriteriosManufactura(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void Anular()
        {
            ...
            if (this.txtCodigo.Text.Trim() != string.Empty && this.txtDescripcion.Text.Trim() != string.Empty)
            {
                if (ValidateForm() == true)
                {
                    ...
                    itemRegistar.idCriterio = Convert.ToInt32(this.txtCodigo.Text);
                    model = new SAS_FormatosDeInspeccionCalidadPacking();
                    if (model.ToChangeStatusCriteriosManufactura(connection, itemRegistar) > 0)
                    {
                        MessageBox.Show("Operacion realizada satisfactoriamente", "Confirmación del sistema");
                        bgwHilo.RunWorkerAsync();
                        gbEdit.Enabled = false;
        ...
        private void EjecuarConsultaAsincrona()
        {
            ...
            try
            {
                ...
                resultList = new List<SAS_CALIDAD_CRITERIOS_MANUFACTURA>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAllCriteriosManufactura(connection).ToList();
                #endregion
            }
            catch (Exception Ex)
            {
