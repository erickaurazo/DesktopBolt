...
using ComparativoHorasVisualSATNISIRA.SIG.Calidad_Packing_Post_Cosecha.BPM._013;
...
{
    public partial class AmonestacionesIncumplimientoDeBuenasPracticasPackingReporte : Form
    {
        ...
        private SAS_ListadoAmonestacionesIncumplimientosByDatesResult selectedItem;
        private List<SAS_ListadoAmonestacionesIncumplimientosByDatesResult> resultList;
        ...
        public AmonestacionesIncumplimientoDeBuenasPracticasPackingReporte()
        {
            InitializeComponent();
            connection = "SAS";
        ...
        public AmonestacionesIncumplimientoDeBuenasPracticasPackingReporte(string _connection, SAS_USUARIOS _userLogin, string _companyId, PrivilegesByUser _privilege)
        {
            InitializeComponent();
        ...
        private void AmonestacionesIncumplimientoDeBuenasPracticasPackingReporte_Load(object sender, EventArgs e)
        {
        ...
        private void VistaPrevia(SAS_ListadoAmonestacionesIncumplimientosByDatesResult selectedItem)
        {
            if (selectedItem != null)
            {
                if (selectedItem.CabeceraId != null && selectedItem.CabeceraId > 0)
                {
                    ...
                    AmonestacionesIncumplimientoDeBuenasPracticasPackingPreview ofrm = new AmonestacionesIncumplimientoDeBuenasPracticasPackingPreview(connection, selectedItem);
                    ofrm.Show();
                }
            }
        }
        ...
        private void LoadFreightSummary()
        {
            ...
            items1.Add(new GridViewSummaryItem("chColaboradorEvaluado", "Count : {0:N2}; ", GridAggregateFunction.Count));
        ...
        private void EjecutarConsultaAsincrona()
        {
            try
            {
                resultList = new List<SAS_ListadoAmonestacionesIncumplimientosByDatesResult>();
                model = new SAS_FormatosDeInspeccionCalidadPacking();
                resultList = model.ListAmonestacionesIncumplimientosByDates(connection, PeriodoDesde, PeriodoHasta, resumido);
            }
            catch (Exception Ex)
            {
                MessageBox.Show(Ex.Message.ToString(), "MENSAJE DEL SISTEMA");
