...
{
    public partial class DistribucionDeCargaEnPackingListDetalle : Form
    {
        ...
        private SAS_ListadoDePackingListAllResult itemSelecionadoFormularioPadre;

        private List<SAS_ListadoConformidadCargaByBookingResult> listadoDeDistribucionDeCarga;


        private int VerSoloCultivoVID = 0;
        private int VerSoloExportable = 0;

        public DistribucionDeCargaEnPackingListDetalle()
        {
            InitializeComponent();
        }

        public DistribucionDeCargaEnPackingListDetalle(string _conection, SAS_USUARIOS _user2, string _companyId, PrivilegesByUser _privilege, SAS_ListadoDePackingListAllResult _itemSelecionadoFormularioPadre)
        {
            InitializeComponent();
            ...
            this.txtBooking.Text = itemSelecionadoFormularioPadre.Booking != null ? itemSelecionadoFormularioPadre.Booking.Trim() : string.Empty;
            Consultar();
        }
        ...
        {
            this.dgvPalletADistribuir.TableElement.BeginUpdate();
            this.LoadFreightSummary();
            this.dgvPalletADistribuir.TableElement.EndUpdate();
        ...
        private void LoadFreightSummary()
        {
            ...
            this.dgvPalletADistribuir.GroupDescriptors.Clear();
            this.dgvPalletADistribuir.GroupDescriptors.Add(new GridGroupByExpression("CustomerID Group By CustomerID"));
            GridViewSummaryRowItem items1 = new GridViewSummaryRowItem();
            items1.Add(new GridViewSummaryItem("chnumpaleta", "Count : {0:N2}; ", GridAggregateFunction.Count));
            this.dgvPalletADistribuir.MasterTemplate.SummaryRowsTop.Add(items1);
        }
            ...
            if (dgvPalletADistribuir.Rows.Count > 0)
            {
                if (dgvPalletADistribuir.CurrentRow != null && dgvPalletADistribuir.CurrentRow.Cells["chCodigo"].Value != null)
                {
                    ...
                    switch (valor)
                    {
                        ...
                        case 21:
                            dgvPalletADistribuir.CurrentRow.Cells["chfila"].Value = 11;
                            break;
                        ...
                        case 23:
                            dgvPalletADistribuir.CurrentRow.Cells["chfila"].Value = 12;
                            break;
        ...
        private void ActualizarDistribucion()
        {
            ...
            try
            {
                if (Validar() == true)
                {
                    gbDetalle.Enabled = false;
                    GbDatosGeneralesPCK.Enabled = false;
                    pbDetalle.Enabled = true;
                    bgwActualizarDatos.RunWorkerAsync();
        ...
        private void btnImportarDistribuirDeCarga_Click(object sender, EventArgs e)
        {
            ImportarDistribucionDeCarga();
        }

        private void ImportarDistribucionDeCarga()
        {
            try
            {
                gbDetalle.Enabled = false;
                GbDatosGeneralesPCK.Enabled = false;
                pbDetalle.Visible = true;
                bgwImportarDistribuicion.RunWorkerAsync();
            }
            catch (Exception Ex)
            {
                MessageBox.Show(Ex.Message, "MENSAJE DEL SISTEA");
        ...
        private void bgwImportarDistribuicion_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {

                modelo = new PackingListController();
                listadoDeDistribucionDeCarga = new List<SAS_ListadoConformidadCargaByBookingResult>();
                if (itemSelecionadoFormularioPadre != null)
                {
                    if (itemSelecionadoFormularioPadre.Booking != null)
                    {
                        if (itemSelecionadoFormularioPadre.Booking.Trim() != string.Empty)
                        {
                            listadoDeDistribucionDeCarga = modelo.listadDeDistribucionDeCargaBolt(conection, itemSelecionadoFormularioPadre.Booking);
                        }

                        if (listadoDeDistribucionDeCarga != null)
                        {
                            if (listadoDeDistribucionDeCarga.ToList().Count > 0)
                            {
                                if (listado != null)
                                {
                                    ...
                                    {
                                        modelo = new PackingListController();
                                        listado = modelo.GenerarListadoConformidadDesdeDistribucionDeCarga(listado, listadoDeDistribucionDeCarga).ToList();
                                    }
                                }
        ...
        private void bgwImportarDistribuicion_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                MessageBox.Show("Sincronización realizada correctamente", "Confirmación del sistema");
                dgvPalletADistribuir.DataSource = listado.ToDataTable<SAS_ObtenerListadoPalletDisponiblesByCodigoPLResult>();
                dgvPalletADistribuir.Refresh();
                gbDetalle.Enabled = true;
                GbDatosGeneralesPCK.Enabled = true;
                pbDetalle.Visible = !true;
            }
            catch (Exception Ex)
            {
        ...
        private void bgwActualizarDatos_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                PresentarResultados();
                MessageBox.Show("Se ha realizado la actualización correctamente el Packing List", "Confirmación del Sistema");             
            }
            catch (Exception Ex)
            {
                MessageBox.Show(Ex.Message.ToString(), "ADVERTENCIA DEL SISTEMA");
